...
class Photo {
	...
	function reload() {
		...
		if ($this->_pid != 0) {
			...
			if ($r == 0)
				throw new Exception2("Bd", "Zdj�ie nie istnieje", PHOTO_NOT_FOUND);
			...
			if (PEAR::isError($r))
				throw new Exception2("Bd wewn�rzny", $r->getMessage());
			if ($r == 0)
				throw new Exception2("Bd sp�noci danych", "Uytkownik do kt�ego nalezy zdj�ie nie istnieje.<br />Skontakuj si�z administratorem, podajc numer zdj�ia ($pid).", PHOTO_USER_NOT_FOUND);
...
Twoje zdj�ie "$title" zostao wanie zaakceptowane przez moderatora ($moderator).

-- 
Ten email zosta wysany automatycznie.
Prosimy nie odpowiada�
EOT;
			Utils::mail("Twoje zdj�ie zostao zaakceptowane.", $txt_body, $this->_user->user_email, $this->_user->user_name);
		}
